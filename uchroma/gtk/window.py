...
from .widgets import MatrixPreview, BrightnessScale
from .widgets.effect_card import EFFECTS, get_effect_by_id
from .widgets.layer_row import RENDERERS, get_renderer_by_id
from .panels import ModeToggle, EffectSelector, LayerPanel, ParamInspector
...
class UChromaWindow(Adw.ApplicationWindow):
    ...
    def _show_layer_params(self, row):
        ...
        if renderer_data:
            self._param_inspector.set_params(
                renderer_data.get('params', []),
                f'{renderer_data["name"].upper()} SETTINGS'
            )
    ...
    def _on_brightness_changed(self, scale, value):
        ...
        if app and hasattr(app, 'dbus'):
            import asyncio
            asyncio.create_task(
                app.dbus.set_brightness(self._device.path, value)
            )
    ...
    def _on_power_toggled(self, btn):
        ...
        if app and hasattr(app, 'dbus'):
            import asyncio
            asyncio.create_task(
                app.dbus.set_suspended(self._device.path, suspended)
            )
