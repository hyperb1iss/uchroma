...
class DashboardPage(Adw.PreferencesPage):
    ...
    def set_device(self, device):
        ...
        device.bind_property('brightness', self.brightness_scale.get_adjustment(), 'value',
                            GObject.BindingFlags.BIDIRECTIONAL | GObject.BindingFlags.SYNC_CREATE)

        # Bind suspend
        device.bind_property('suspended', self.suspend_row, 'active',
                            GObject.BindingFlags.BIDIRECTIONAL | GObject.BindingFlags.SYNC_CREATE)
    ...
    def _on_quick_effect(self, button, effect_id):
        ...
        if app:
            import asyncio
            asyncio.create_task(
                app.dbus.set_effect(self._device.path, effect_id)
            )
