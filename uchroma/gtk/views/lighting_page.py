...
RENDERERS = [
    ...
    },
    {
        ...
        'icon': 'weather-clear-symbolic',
        'params': [
            {'name': 'speed', 'type': 'range', 'label': 'Speed', 'min': 0.1, 'max': 2.0,
             'step': 0.1, 'default': 1.0},
            {'name': 'saturation', 'type': 'range', 'label': 'Saturation', 'min': 0.0, 'max': 1.0,
    ...
    },
    {
        ...
        'params': [
            ...
            {'name': 'speed', 'type': 'range', 'label': 'Speed', 'min': 0.5, 'max': 3.0,
    ...
    },
    {
        ...
        'params': [
            ...
            {'name': 'fade_time', 'type': 'range', 'label': 'Fade Time', 'min': 0.1, 'max': 2.0,
...
BLEND_MODES = [
    'normal', 'screen', 'multiply', 'addition', 'subtract',
    'lighten_only', 'darken_only', 'soft_light', 'hard_light',
    'dodge', 'difference', 'grain_extract', 'grain_merge', 'divide'
]
...
class LightingPage(Adw.PreferencesPage):
    ...
    def _build_ui(self):
        ...
        mode_row.set_subtitle('Hardware effects run on device, custom animation is software-rendered')
    ...
    def _on_renderer_chosen(self, dialog, response):
        ...
        if self._device:
            ...
            if app:
                import asyncio
                asyncio.create_task(
                    app.dbus.add_renderer(self._device.path, response, zindex)
                )
    ...
    def _on_layer_delete(self, layer_row, row):
        ...
        if self._device:
            ...
            if app:
                import asyncio
                asyncio.create_task(
                    app.dbus.remove_renderer(self._device.path, zindex)
                )
    ...
    def _build_layer_settings(self, layer_row: LayerRow):
        ...
        icon = Gtk.Image.new_from_icon_name(renderer_data.get('icon', 'applications-graphics-symbolic'))
    ...
    def _stop_animation(self):
        ...
        if self._device:
            ...
            if app:
                import asyncio
                asyncio.create_task(
                    app.dbus.stop_animation(self._device.path)
                )
