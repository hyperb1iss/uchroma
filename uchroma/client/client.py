...
def color_block(*values):
    ...
    for value in values:
        ...
        output = output.center(
            9, text=col.html, fore=ColorUtils.inverse(col).intTuple, back=col.intTuple
        )
    return str(output)
...
class AbstractCommand:
    ...
    def list_objects(self, objects, values=None, aliases=None, show_all: bool = False, keylen=None):
        if aliases is None:
            keys = objects.keys()
        else:
        ...
        for name, trait_data in objects.items():
            ...
            self.columns(keylen, color(dname, style="bright"), color(desc, style="bright"))
...
class DumpCommand(AbstractCommand):
    ...
    def parse(self, args):
        ...
        self.columns(
            keylen, color(device_index, style="bright"), color(device_name, style="bright")
        )
...
class FXCommand(AbstractCommand):
    ...
    def available_fx(self):
        ...
        for name, t_dict in sorted(self.driver.AvailableFX.items()):
            ...
            self._fx[name] = RemoteTraits(name, traits.description, None, None, traits)
...
class AnimationCommand(AbstractCommand):
    ...
    def parse(self, args):
        ...
        delete = sub.add_parser(
            "delete", help="Delete a layer from the stack", aliases=["del"], add_help=False
        )
        delete.set_defaults(func_anim=self._delete, parser=delete)

        modify = sub.add_parser(
            "modify", help="Modify an existing layer", aliases=["mod"], add_help=False
        )
    ...
    def renderer_info(self):
        ...
        sar = OrderedDict(
            sorted(avail.items(), key=lambda k_v: get_meta_field(k_v[1]["meta"], "display_name", 0))
        )
        ...
        for name, t_anim in sar.items():
            ...
            rt = RemoteTraits(
                get_meta_field(meta, "display_name", 0),
    ...
    def dump(self, keylen):
        if len(self.current_state) > 0:
            ...
            self.list_objects(
                self.renderer_info, self.current_state, aliases=self._aliases, keylen=keylen
            )

    def _list(self, args):
        print("\n Available renderers and arguments:\n")
        self.list_objects(
            self.renderer_info, values=self.current_state, aliases=self._aliases, show_all=True
        )
    ...
    def _add(self, args):
        ...
        args.parser.add_argument(
            ...
            "--zindex",
            type=int,
            choices=range(len(layers) + 1),
            help="Z-index for the layer",
            dest="zzz",
        )
    ...
    def _delete(self, args):
        ...
        args.parser.add_argument(
            "zindex", type=int, choices=range(len(layers)), help="Layer index to be removed"
        )
